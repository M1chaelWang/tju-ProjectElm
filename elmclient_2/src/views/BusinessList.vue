<template>
	  ...
	  <ul class="business">
		<li v-for="item in businessArr" :key="item.businessId" @click="toBusinessInfo(item.businessId)">
		  <div class="business-img">
			<img :src="item.businessImg" alt="Business Image">
  ...
  export default {
	...
	created() {
	  ...
	  // 根据orderTypeId查询商家信息
	  this.$axios
		.post('BusinessController/listBusinessByOrderTypeId', this.$qs.stringify({
	...
	methods: {
	  async listCart() {
		try {
		  const response = await this.$axios.post('CartController/listCart', this.$qs.stringify({
