        ...
        <ul class="addresslist">
            <li v-for="item in deliveryAddressArr">
                <div class="addresslist-left" @click="setDeliveryAddress(item)">
                    <h3>
                        {{ item.contactName
                        }}{{ this.sexFilter(item.contactSex) }}
                        {{ item.contactTel }}
                    </h3>
                    <p>{{ item.address }}</p>
                </div>
...
export default {
    ...
    methods: {
        ...
        listDeliveryAddressByUserId() {
            //查询送货地址
            this.$axios
                .get(`deliveryAddresses/user/${this.user.userId}`, this.config)
                .then((response) => {
        ...
        removeUserAddress(daId) {
            ...
            this.$axios
                .delete(`deliveryAddresses/${daId}`, this.config)
                .then((response) => {
                    if (response.data > 0) {
                        let deliveryAddress = this.$getLocalStorage(
                            this.user.userId
                        );
                        if (
